        ...
        for (const queue of ['auth_emails', 'transactional_emails']) {
          ...
          for (let i = 0; i < messages.length; i++) {
            ...
            if (payload.to && typeof payload.to === 'string') {
              ...
              const { data: suppressedNow, error: suppressionCheckError } = await supabase
              ...
              if (suppressionCheckError) {
                console.error('Queue suppression check failed', {
                  queue,
                  msg_id: msg.msg_id,
                  error: suppressionCheckError,
                })
                // Leave in queue to retry on next tick.
                continue
              }
              ...
              if (suppressedNow) {
                await supabase.from('email_send_log').insert({
                  ...
                  recipient_email: payload.to,
                  status: 'suppressed',
                  error_message: 'Recipient unsubscribed after enqueue',
                })
                ...
                if (supDelError) {
                  console.error('Failed to delete suppressed message from queue', {
                    queue, msg_id: msg.msg_id, error: supDelError,
                  })
                }
                continue
              }
